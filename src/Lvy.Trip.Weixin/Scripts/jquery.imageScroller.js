...
jQuery.fn.imageScroller = function(params){
	...
	var turnLeft = function(){
	    _btnPrev.unbind("mouseover", turnLeft);
		if(_auto) autoStop();
		_imgFrame.animate( {marginLeft:-_width}, 'fast', '', function(){
			_imgFrame.find(_child+":first").appendTo( _imgFrame );
			_imgFrame.css("marginLeft",0);
			_btnPrev.bind("mouseover", turnLeft);
			if(_auto) autoPlay();
		});
	};
	
	var turnRight = function(){
	    _btnNext.unbind("mouseover", turnRight);
		if(_auto) autoStop();
		_imgFrame.find(_child+":last").clone().show().prependTo( _imgFrame );
		_imgFrame.css("marginLeft",-_width);
		_imgFrame.animate( {marginLeft:0}, 'fast' ,'', function(){
			_imgFrame.find(_child+":last").remove();
			_btnNext.bind("mouseover", turnRight);
			if(_auto) autoPlay(); 
		});
	};

_btnNext.css("cursor", "hand").mouseover(turnRight);
_btnPrev.css("cursor", "hand").mouseover(turnLeft);
