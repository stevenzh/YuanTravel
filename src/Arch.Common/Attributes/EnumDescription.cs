...
namespace Arch.Common.Attributes
{
    /// <summary>
    /// 处理枚举与数据库状态值之间的转换
    /// </summary>
    ...
    public class EnumDescription : Attribute
    {
        ...
        private static System.Collections.Hashtable _cachedEnum = new Hashtable();
        ...
        {
            EnumDescription[] eds =
                (EnumDescription[])enumType.GetCustomAttributes(typeof(EnumDescription), false);
        ...
        public static string GetFieldText(Type enumType, object value)
        {
            EnumDescription[] descriptions = GetFieldTexts(enumType, SortType.Default);
            foreach (EnumDescription ed in descriptions)
            {
                if (ed.EnumValue == value.ToInt()) return ed.EnumDisplayText;
            }
            return string.Empty;
        }

        public static string GetFieldName(Type enumType, object value)
        {
            EnumDescription[] descriptions = GetFieldTexts(enumType, SortType.Default);
            foreach (EnumDescription ed in descriptions)
            {
                if (ed.FieldName == value.ToString()) return ed.EnumDisplayText;
            }
            return string.Empty;
        }
        ...
        public static EnumDescription[] GetFieldTexts(Type enumType, SortType sortType)
        {
            ...
            if (_cachedEnum.Contains(enumType.FullName) == false)
            {
                ...
                _cachedEnum.Add(enumType.FullName, (EnumDescription[])edAL.ToArray(typeof(EnumDescription)));
            }
            descriptions = (EnumDescription[])_cachedEnum[enumType.FullName];
        ...
        public const string ValueField = "EnumValue";
        public const string TextField = "EnumDisplayText";
        public const string EnumNameField = "FieldName";
